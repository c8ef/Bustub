 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    if ((*ptr)->HasChild(key.back())) {
      auto child = (*ptr)->GetChildNode(key.back());
      if ((*child)->IsEndNode()) {
        latch_.WUnlock();
        return false;
      }
    ...
    (*ptr)->InsertChildNode(key.back(), std::make_unique<TrieNodeWithValue<T>>(key.back(), value));
    latch_.WUnlock();
    return true;
  }
  ...
  bool Remove(const std::string &key) {
    ...
    auto ptr = &root_;
    std::vector<std::unique_ptr<TrieNode> *> parent;
    ...
    for (int i = parent.size() - 1; i >= 0; --i) {
      if (!(*child)->HasChildren()) {
        (*parent[i])->RemoveChildNode(key[i]);
      }
      child = parent[i];
    }
    latch_.WUnlock();
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    auto cast_ptr = dynamic_cast<TrieNodeWithValue<T> *>((*(*ptr)->GetChildNode(key.back())).get());
    if (!cast_ptr) {
      latch_.RUnlock();
      *success = false;
      return {};
